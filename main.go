...
func queryHTTPSRecord(domain, dnsServer string) (string, error) {
	// 检查是否为 IP:端口 格式（UDP DNS）
	if _, _, err := net.SplitHostPort(dnsServer); err == nil {
		// 是 IP:端口 格式，使用 UDP
		return queryHTTPSRecordUDP(domain, dnsServer)
	}
	
	// 不是 IP:端口 格式，作为 DoH URL 处理
	// 如果没有 http:// 或 https:// 前缀，自动添加 https://
	dohURL := dnsServer
	if !strings.HasPrefix(dohURL, "http://") && !strings.HasPrefix(dohURL, "https://") {
		dohURL = "https://" + dohURL
	}
	
	return queryHTTPSRecordDoH(domain, dohURL)
}

// DoH (DNS over HTTPS) 查询 - 修复版本
...
func runProxyServer(addr string) {
	...
	if _, _, err := net.SplitHostPort(dnsServer); err == nil {
		...
		log.Printf("[DNS] 使用 DoH: %s", dohURL)
	}
